    ...
    public partial class T_AddressController : BaseController
    {
	 ...
	 if(sortBy == "T_AddressCityID")
        ...
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_addresscountry,string t_addressstate,string t_addresscity, bool? RenderPartial)
        {
			...
			{
				ViewBag.t_addresscountry = new SelectList(T_AddressCountrylist, "ID", "DisplayValue");
			}
			...
			{
				ViewBag.t_addressstate = new SelectList(T_AddressStatelist, "ID", "DisplayValue");
			}
			...
			{
				ViewBag.t_addresscity = new SelectList(T_AddressCitylist, "ID", "DisplayValue");
			}
			}
			else
			{
			...
			columns.Add("7", "Address City");
				...
				if ((RenderPartial == null ? false : RenderPartial.Value))
					return PartialView("~/Views/T_Address/CustomReportSearch.cshtml", new T_Address());
		...
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_addresscountry,string t_addressstate,string t_addresscity , string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports)
        {
			...
			lstT_Address = Sorting.Sort<T_Address>(lstT_Address, SortOrder);
			 ...
			 if (!string.IsNullOrEmpty(GroupByColumn))
            {
                ...
                foreach (string prop in props)
                {
                    ...
                    var asso = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Address").Associations.FirstOrDefault(p => p.AssociationProperty == prop);
                    if (asso != null)
                    {
                        columns.Add(prop);
                        if (iCnt == 1)
                ...
                var grp_T_Address = lstT_Address;
                var grp_T_AddressList = grp_T_Address.GroupByMany(groupcolumn);
                var objT_AddressList = new List<T_Address>();

                foreach (var grp in grp_T_AddressList)
                {
                    ...
                    if (Subgroup != null && Subgroup.Count() > 0)
                    {
                        foreach (var itemsub in Subgroup)
                        {
                            ...
                            {
                                var objT_Address = new T_Address();
                                objT_Address = (T_Address)item1;
                                objT_Address.m_DisplayValue = SecondKey;
                                objT_AddressList.Add(objT_Address);
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            var objT_Address = new T_Address();
                            objT_Address = (T_Address)itemgrp;
                            objT_Address.m_DisplayValue = firstKey.ToString();
                            objT_AddressList.Add(objT_Address);
                        }
                    }
                }
				if (objT_AddressList.Count() > 0)
                lstT_Address = objT_AddressList.AsQueryable();
           ...
           // return View("Index", _T_Address.ToPagedList(pageNumber, pageSize));
		    if (IsReports != null)
            {
			  var lstCustReportItem = db.FavoriteItems.Where(p => p.LastUpdatedByUser == User.Name);
                ...
                return PartialView("~/Views/T_Address/CustomReportResult.cshtml", _T_Address.ToPagedList(pageNumber, pageSize));
            }
	...
	break;
        ...
        public JsonResult GetValidateBeforeSaveProperties(T_Address OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Address");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		public string CheckBeforeSave(T_Address t_address)
        {
			var AlertMsg = "";
            // Write your logic here
