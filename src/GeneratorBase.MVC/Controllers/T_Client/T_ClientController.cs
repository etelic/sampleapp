...
{	
    ...
    {
		private void LoadViewDataForCount(T_Client t_client, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_Client t_client)
        {
		        }
        private void LoadViewDataBeforeOnEdit(T_Client t_client)
        {
		
        }
        private void LoadViewDataAfterOnCreate(T_Client t_client)
        {
        }
		...
		private IQueryable<T_Client> searchRecords(IQueryable<T_Client> lstT_Client, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_Client = lstT_Client.Where(s => (!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_Client = lstT_Client.Where(s => (!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_Client;
        }
		private IQueryable<T_Client> sortRecords(IQueryable<T_Client> lstT_Client, string sortBy, string isAsc)
        {
            string methodName = "";
            ...
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_Client), "t_client");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_Client>)lstT_Client.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_Client.ElementType, lambda.Body.Type },
                    lstT_Client.Expression,
                    lambda));
        }
        ...
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_sessionclientassociation,string t_sessioneventsclient, bool? RenderPartial)
        {
			...
			if(Qcount > 0)
			{
			var T_SessionClientAssociationlist = db.T_Sessions.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
                ...
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Session= ";
				  ...
				  foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_SessionClientAssociationlist.Union(db.T_Sessions.Where(p=>ids1.Contains(p.Id))).Distinct();
			...
			var T_SessionEventsClientlist = db.T_SessionEventss.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
                ...
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Session Events= ";
				  ...
				  foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_SessionEventsClientlist.Union(db.T_SessionEventss.Where(p=>ids1.Contains(p.Id))).Distinct();
			...
			{
			var objT_SessionClientAssociation = new List<T_Session>();
		    ViewBag.t_sessionclientassociation = new SelectList(objT_SessionClientAssociation, "ID", "DisplayValue");
			var objT_SessionEventsClient = new List<T_SessionEvents>();
			...
			columns.Add("3", "Description");
				...
				if ((RenderPartial == null ? false : RenderPartial.Value))
					return PartialView("~/Views/T_Client/CustomReportSearch.cshtml", new T_Client());
				return View(new T_Client());
            }

		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_Client", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_Client", value, true);
		...
		// GET: /T_Client/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_sessionclientassociation,string t_sessioneventsclient , string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports)
        {
            ...
            {
                lstT_Client  = searchRecords(lstT_Client, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_Client = searchRecords(lstT_Client, search,true);
			}
			...
			lstT_Client = lstT_Client;
			lstT_Client = Sorting.Sort<T_Client>(lstT_Client, SortOrder);
			...
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    ViewBag.SearchResult += " " + GetPropertyDP("T_Client", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_Client = lstT_Client.Where(whereCondition.ToString());
			 ...
			 if (!string.IsNullOrEmpty(GroupByColumn))
            {
                ...
                foreach (string prop in props)
                {
                    ...
                    var asso = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Client").Associations.FirstOrDefault(p => p.AssociationProperty == prop);
                    if (asso != null)
                    {
                        columns.Add(prop);
                        if (iCnt == 1)
                ...
                var grp_T_Client = lstT_Client;
                var grp_T_ClientList = grp_T_Client.GroupByMany(groupcolumn);
                var objT_ClientList = new List<T_Client>();

                foreach (var grp in grp_T_ClientList)
                {
                    ...
                    if (Subgroup != null && Subgroup.Count() > 0)
                    {
                        foreach (var itemsub in Subgroup)
                        {
                            ...
                            {
                                var objT_Client = new T_Client();
                                objT_Client = (T_Client)item1;
                                objT_Client.m_DisplayValue = SecondKey;
                                objT_ClientList.Add(objT_Client);
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            var objT_Client = new T_Client();
                            objT_Client = (T_Client)itemgrp;
                            objT_Client.m_DisplayValue = firstKey.ToString();
                            objT_ClientList.Add(objT_Client);
                        }
                    }
                }
				if (objT_ClientList.Count() > 0)
                lstT_Client = objT_ClientList.AsQueryable();
			...
			if (t_sessionclientassociation != null)
            {
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        ViewBag.SearchResult += db.T_Sessions.Find(Convert.ToInt64(str)).DisplayValue + ", ";
                        ids1.AddRange(db.T_SessionClientAssociations.Where(p=>p.T_SessionID ==idvalue).Select(p=>p.T_ClientID));
                        }
                ...
                ids1 = ids1.ToList();
                _T_Client = _T_Client.Where(p => ids1.Contains(p.Id));
            }
			if (t_sessioneventsclient != null)
            {
                var ids = t_sessioneventsclient.Split(",".ToCharArray());
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        var idvalue= Convert.ToInt64(str);
                        ViewBag.SearchResult += db.T_SessionEventss.Find(Convert.ToInt64(str)).DisplayValue + ", ";
                        ids1.AddRange(db.T_SessionEventsClients.Where(p=>p.T_SessionEventsID ==idvalue).Select(p=>p.T_ClientID));
                        }
			...
			if (!string.IsNullOrEmpty(SortOrder))
            {
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Client");
           ...
           // return View("Index", _T_Client.ToPagedList(pageNumber, pageSize));
		    if (IsReports != null)
            {
			  var lstCustReportItem = db.FavoriteItems.Where(p => p.LastUpdatedByUser == User.Name);
                ...
                return PartialView("~/Views/T_Client/CustomReportResult.cshtml", _T_Client.ToPagedList(pageNumber, pageSize));
            }
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Clients.FirstOrDefault(p => p.Id == idvalue);
		 ...
		 public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Clients.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Clients.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Client>(_Obj, "T_Client", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_Client> list = db.T_Clients;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                if (AssoID != null && AssoID > 0)
                {
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_Client), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_Client, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_Client>)q);
				   list = ((IQueryable<T_Client>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Clients;
            ...
            list = _fad.FilterDropdown<T_Client>(User,list, "T_Client",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_Client>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
                ...
                ParameterExpression param = Expression.Parameter(typeof(T_Client), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_Client).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_Client), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Client), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Client), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Client), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Client), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Client), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_Client), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_Client), "p"));
        ...
        {
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Client")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Client").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_Client");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_Client" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Clients";
	...
	break;
		...
		public bool ValidateModel(T_Client validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_Clients.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_Client OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_Client").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_Client");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_Client OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_Client");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_Client OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Client");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_Client OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_Client");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_Client OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Client").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Clients.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_Clients;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_Client, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_Client, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Client>)q);
		...
		public string CheckBeforeSave(T_Client t_client)
        {
			var AlertMsg = "";
            // Write your logic here
		...
		public void OnDeleting(T_Client t_client)
        {
            // Write your logic here
 
		}
        public void OnSaving(T_Client t_client, ApplicationContext db)
        {
            // Write your logic here
 
        }
		public void AfterSave(T_Client t_client)
        {
        ...
        public ActionResult GetCalculationValues(T_Client t_client)
        {
            t_client.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_client.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_Client t_client, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_Client t_client, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
