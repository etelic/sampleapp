...
{	
    ...
    {
		private void LoadViewDataForCount(T_Organization t_organization, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_Organization t_organization)
        {
		        }
        private void LoadViewDataBeforeOnEdit(T_Organization t_organization)
        {
		
        }
        private void LoadViewDataAfterOnCreate(T_Organization t_organization)
        {
        }
		...
		private IQueryable<T_Organization> sortRecords(IQueryable<T_Organization> lstT_Organization, string sortBy, string isAsc)
        {
            string methodName = "";
            ...
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_Organization), "t_organization");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_Organization>)lstT_Organization.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_Organization.ElementType, lambda.Body.Type },
                    lstT_Organization.Expression,
                    lambda));
        }
        ...
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_employeeorganizationassociation, bool? RenderPartial)
        {
			...
			if(Qcount > 0)
			{
			var T_EmployeeOrganizationAssociationlist = db.T_Employees.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
                ...
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employee= ";
				  ...
				  foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_EmployeeOrganizationAssociationlist.Union(db.T_Employees.Where(p=>ids1.Contains(p.Id))).Distinct();
			...
			{
			var objT_EmployeeOrganizationAssociation = new List<T_Employee>();
		    ViewBag.t_employeeorganizationassociation = new SelectList(objT_EmployeeOrganizationAssociation, "ID", "DisplayValue");
				...
				ViewBag.EntityName = "T_Organization";
			...
			columns.Add("3", "Description");
				...
				if ((RenderPartial == null ? false : RenderPartial.Value))
					return PartialView("~/Views/T_Organization/CustomReportSearch.cshtml", new T_Organization());
				return View(new T_Organization());
            }

		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_Organization", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_Organization", value, true);
		...
		// GET: /T_Organization/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_employeeorganizationassociation , string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports)
        {
            ...
            {
                lstT_Organization  = searchRecords(lstT_Organization, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_Organization = searchRecords(lstT_Organization, search,true);
			}
			...
			lstT_Organization = lstT_Organization;
			lstT_Organization = Sorting.Sort<T_Organization>(lstT_Organization, SortOrder);
			...
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    ViewBag.SearchResult += " " + GetPropertyDP("T_Organization", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_Organization = lstT_Organization.Where(whereCondition.ToString());
			 ...
			 if (!string.IsNullOrEmpty(GroupByColumn))
            {
                ...
                foreach (string prop in props)
                {
                    ...
                    var asso = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Organization").Associations.FirstOrDefault(p => p.AssociationProperty == prop);
                    if (asso != null)
                    {
                        columns.Add(prop);
                        if (iCnt == 1)
                ...
                var grp_T_Organization = lstT_Organization;
                var grp_T_OrganizationList = grp_T_Organization.GroupByMany(groupcolumn);
                var objT_OrganizationList = new List<T_Organization>();

                foreach (var grp in grp_T_OrganizationList)
                {
                    ...
                    if (Subgroup != null && Subgroup.Count() > 0)
                    {
                        foreach (var itemsub in Subgroup)
                        {
                            ...
                            {
                                var objT_Organization = new T_Organization();
                                objT_Organization = (T_Organization)item1;
                                objT_Organization.m_DisplayValue = SecondKey;
                                objT_OrganizationList.Add(objT_Organization);
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            var objT_Organization = new T_Organization();
                            objT_Organization = (T_Organization)itemgrp;
                            objT_Organization.m_DisplayValue = firstKey.ToString();
                            objT_OrganizationList.Add(objT_Organization);
                        }
                    }
                }
				if (objT_OrganizationList.Count() > 0)
                lstT_Organization = objT_OrganizationList.AsQueryable();
			...
			if (t_employeeorganizationassociation != null)
            {
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        ViewBag.SearchResult += db.T_Employees.Find(Convert.ToInt64(str)).DisplayValue + ", ";
                        ids1.AddRange(db.T_EmployeeOrganizationAssociations.Where(p=>p.T_EmployeeID ==idvalue).Select(p=>p.T_OrganizationID));
                        }
                ...
                ids1 = ids1.ToList();
                _T_Organization = _T_Organization.Where(p => ids1.Contains(p.Id));
            }
			if (!string.IsNullOrEmpty(SortOrder))
            {
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Organization");
           ...
           // return View("Index", _T_Organization.ToPagedList(pageNumber, pageSize));
		    if (IsReports != null)
            {
			  var lstCustReportItem = db.FavoriteItems.Where(p => p.LastUpdatedByUser == User.Name);
                ...
                return PartialView("~/Views/T_Organization/CustomReportResult.cshtml", _T_Organization.ToPagedList(pageNumber, pageSize));
            }
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Organizations.FirstOrDefault(p => p.Id == idvalue);
		 ...
		 public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Organizations.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Organizations.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Organization>(_Obj, "T_Organization", new string[] { ""  }); ;
			        }
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_Organizations;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_Organization), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_Organization, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_Organization>)q);
				   list = ((IQueryable<T_Organization>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Organizations;
            ...
            list = _fad.FilterDropdown<T_Organization>(User,list, "T_Organization",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_Organization>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
                ...
                ParameterExpression param = Expression.Parameter(typeof(T_Organization), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_Organization).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_Organization), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Organization), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Organization), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Organization), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Organization), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Organization), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_Organization), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_Organization), "p"));
        ...
        {
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Organization")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Organization").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_Organization");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_Organization" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Organizations";
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_Organization";
	...
	break;
		...
		public bool ValidateModel(T_Organization validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_Organizations.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_Organization OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_Organization").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_Organization");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_Organization OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_Organization");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_Organization OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Organization");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_Organization OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_Organization");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_Organization OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Organization").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Organizations.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_Organizations;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_Organization, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_Organization, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Organization>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_Organization t_organization)
        {
            Dictionary<string, string> msgThreshold = new Dictionary<string, string>();
            return Json(msgThreshold, JsonRequestBehavior.AllowGet);
        }
		public bool CheckBeforeDelete(T_Organization t_organization)
        {
            var result = true;
		...
		public string CheckBeforeSave(T_Organization t_organization)
        {
			var AlertMsg = "";
            // Write your logic here
		...
		public void OnDeleting(T_Organization t_organization)
        {
            // Write your logic here
 
		}
        public void OnSaving(T_Organization t_organization, ApplicationContext db)
        {
            // Write your logic here
 
        }
		public void AfterSave(T_Organization t_organization)
        {
        ...
        public ActionResult GetCalculationValues(T_Organization t_organization)
        {
            t_organization.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_organization.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_Organization"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_Organization t_organization, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_Organization t_organization, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
