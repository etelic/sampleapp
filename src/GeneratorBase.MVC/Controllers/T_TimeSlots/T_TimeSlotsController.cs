...
{	
    ...
    {
		private void LoadViewDataForCount(T_TimeSlots t_timeslots, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_TimeSlots t_timeslots)
        {
					 ViewBag.T_LearningCenterTimeSlotsAssociationID = new SelectList(db.T_LearningCenters, "ID", "DisplayValue", t_timeslots.T_LearningCenterTimeSlotsAssociationID);
        }
        private void LoadViewDataBeforeOnEdit(T_TimeSlots t_timeslots)
        {
		
               var _objT_LearningCenterTimeSlotsAssociation = new List<T_LearningCenter>();
               _objT_LearningCenterTimeSlotsAssociation.Add(t_timeslots.t_learningcentertimeslotsassociation);
			   			   ViewBag.T_LearningCenterTimeSlotsAssociationID = new SelectList(_objT_LearningCenterTimeSlotsAssociation, "ID", "DisplayValue", t_timeslots.T_LearningCenterTimeSlotsAssociationID);
				 ViewBag.T_SessionTimeSlotAssociatonCount = t_timeslots.t_sessiontimeslotassociaton.Count();
	 ViewBag.T_SessionEventsTimeSlotsCount = t_timeslots.t_sessioneventstimeslots.Count();
        }
        private void LoadViewDataAfterOnCreate(T_TimeSlots t_timeslots)
        {
			 ViewBag.T_LearningCenterTimeSlotsAssociationID = new SelectList(db.T_LearningCenters, "ID", "DisplayValue", t_timeslots.T_LearningCenterTimeSlotsAssociationID);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {
			...
			if(HostingEntityName == "T_LearningCenter" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_LearningCenterTimeSlotsAssociation")
			{
			    long hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_LearningCenterTimeSlotsAssociationID = new SelectList(db.T_LearningCenters.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_LearningCenters.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_LearningCenterTimeSlotsAssociation = new List<T_LearningCenter>();
			 ViewBag.T_LearningCenterTimeSlotsAssociationID = new SelectList(objT_LearningCenterTimeSlotsAssociation , "ID", "DisplayValue");
		    }
        }
		private IQueryable<T_TimeSlots> searchRecords(IQueryable<T_TimeSlots> lstT_TimeSlots, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_TimeSlots = lstT_TimeSlots.Where(s => (s.T_SlotNo != null && SqlFunctions.StringConvert((double)s.T_SlotNo).Contains(searchString)) ||(s.t_learningcentertimeslotsassociation!= null && (s.t_learningcentertimeslotsassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_TimeSlots = lstT_TimeSlots.Where(s => (s.T_SlotNo != null && SqlFunctions.StringConvert((double)s.T_SlotNo).Contains(searchString)) ||(s.t_learningcentertimeslotsassociation!= null && (s.t_learningcentertimeslotsassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
			try
            {
                var datevalue = Convert.ToDateTime(searchString);
                lstT_TimeSlots = lstT_TimeSlots.Union(db.T_TimeSlotss.Where(s => (s.T_StartTime == datevalue) ||(s.T_EndTime == datevalue) ));
            }
            catch { }
            return lstT_TimeSlots;
        }
		private IQueryable<T_TimeSlots> sortRecords(IQueryable<T_TimeSlots> lstT_TimeSlots, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_LearningCenterTimeSlotsAssociationID")
                return isAsc.ToLower() == "asc" ? lstT_TimeSlots.OrderBy(p => p.t_learningcentertimeslotsassociation.DisplayValue) : lstT_TimeSlots.OrderByDescending(p => p.t_learningcentertimeslotsassociation.DisplayValue);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_TimeSlots), "t_timeslots");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_TimeSlots>)lstT_TimeSlots.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_TimeSlots.ElementType, lambda.Body.Type },
                    lstT_TimeSlots.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Session")
            {
                ...
                url += "?search=";
						var T_LearningCenterTimeSlotsAssociation  = _Object.T_SessionLearningCenterAssociationID;									
            ...
            if (sourceEntity == "T_SessionEvents")
            {
                ...
                url += "?search=";
						var T_LearningCenterTimeSlotsAssociation  = _Object.T_SessionEventsLearningCenterID;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_learningcentertimeslotsassociation, bool? RenderPartial)
        {
			...
			if(Qcount > 0)
			{
			var T_LearningCenterTimeSlotsAssociationlist = db.T_LearningCenters.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_learningcentertimeslotsassociation != null)
            {
                var ids = t_learningcentertimeslotsassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Learning Center= ";
				   ...
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_LearningCenterTimeSlotsAssociationlist.Union(db.T_LearningCenters.Where(p=>ids1.Contains(p.Id))).Distinct();
					ViewBag.t_learningcentertimeslotsassociation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				ViewBag.t_learningcentertimeslotsassociation = new SelectList(T_LearningCenterTimeSlotsAssociationlist, "ID", "DisplayValue");
			}
			}
			else
			{
			var objT_LearningCenterTimeSlotsAssociation = new List<T_LearningCenter>();
		    ViewBag.t_learningcentertimeslotsassociation = new SelectList(objT_LearningCenterTimeSlotsAssociation, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_TimeSlots";
			...
			columns.Add("2", "SlotNo");
			columns.Add("3", "Learning Center");
			columns.Add("4", "Start Time");
			columns.Add("5", "End Time");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				if ((RenderPartial == null ? false : RenderPartial.Value))
					return PartialView("~/Views/T_TimeSlots/CustomReportSearch.cshtml", new T_TimeSlots());
				return View(new T_TimeSlots());
            }

		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_TimeSlots", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_TimeSlots", value, true);
		...
		// GET: /T_TimeSlots/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_learningcentertimeslotsassociation ,string T_SlotNoFrom,string T_SlotNoTo,string T_StartTimeFrom,string T_StartTimeTo,string T_EndTimeFrom,string T_EndTimeTo, string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports)
        {
            ...
            {
                lstT_TimeSlots  = searchRecords(lstT_TimeSlots, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_TimeSlots = searchRecords(lstT_TimeSlots, search,true);
			}
			...
			lstT_TimeSlots = lstT_TimeSlots.Include(t=>t.t_learningcentertimeslotsassociation);
			lstT_TimeSlots = Sorting.Sort<T_TimeSlots>(lstT_TimeSlots, SortOrder);
			...
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    ViewBag.SearchResult += " " + GetPropertyDP("T_TimeSlots", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_TimeSlots = lstT_TimeSlots.Where(whereCondition.ToString());
			 ...
			 if (!string.IsNullOrEmpty(GroupByColumn))
            {
                ...
                foreach (string prop in props)
                {
                    ...
                    var asso = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_TimeSlots").Associations.FirstOrDefault(p => p.AssociationProperty == prop);
                    if (asso != null)
                    {
                        columns.Add(prop);
                        if (iCnt == 1)
                ...
                var grp_T_TimeSlots = lstT_TimeSlots;
                var grp_T_TimeSlotsList = grp_T_TimeSlots.GroupByMany(groupcolumn);
                var objT_TimeSlotsList = new List<T_TimeSlots>();

                foreach (var grp in grp_T_TimeSlotsList)
                {
                    ...
                    if (Subgroup != null && Subgroup.Count() > 0)
                    {
                        foreach (var itemsub in Subgroup)
                        {
                            ...
                            {
                                var objT_TimeSlots = new T_TimeSlots();
                                objT_TimeSlots = (T_TimeSlots)item1;
                                objT_TimeSlots.m_DisplayValue = SecondKey;
                                objT_TimeSlotsList.Add(objT_TimeSlots);
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            var objT_TimeSlots = new T_TimeSlots();
                            objT_TimeSlots = (T_TimeSlots)itemgrp;
                            objT_TimeSlots.m_DisplayValue = firstKey.ToString();
                            objT_TimeSlotsList.Add(objT_TimeSlots);
                        }
                    }
                }
				if (objT_TimeSlotsList.Count() > 0)
                lstT_TimeSlots = objT_TimeSlotsList.AsQueryable();
            ...
            var _T_TimeSlots = lstT_TimeSlots;
		 if(T_SlotNoFrom!=null || T_SlotNoTo !=null)
         {
                try
                {
                    long from = T_SlotNoFrom == null ? 0 : Convert.ToInt64(T_SlotNoFrom);
                    long to =  T_SlotNoTo == null ? long.MaxValue : Convert.ToInt64(T_SlotNoTo);
                    _T_TimeSlots =  _T_TimeSlots.Where(o => o.T_SlotNo >= from && o.T_SlotNo <= to);
					ViewBag.SearchResult += "\r\n SlotNo= "+from+"-"+to;
                }
                catch { }
          }
				if(T_StartTimeFrom!=null || T_StartTimeTo !=null)
				{
						try
						{
							...
							_T_TimeSlots =  _T_TimeSlots.Where(o => o.T_StartTime >= from && o.T_StartTime <= to);
				...
				if(T_EndTimeFrom!=null || T_EndTimeTo !=null)
				{
						try
						{
							...
							_T_TimeSlots =  _T_TimeSlots.Where(o => o.T_EndTime >= from && o.T_EndTime <= to);
							ViewBag.SearchResult += "\r\n End Time= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
			//if (lstT_TimeSlots.Where(p => p.t_learningcentertimeslotsassociation != null).Count() <= 50)
		    //ViewBag.t_learningcentertimeslotsassociation = new SelectList(lstT_TimeSlots.Where(p => p.t_learningcentertimeslotsassociation != null).Select(P => P.t_learningcentertimeslotsassociation).Distinct(), "ID", "DisplayValue");
            if (t_learningcentertimeslotsassociation != null)
            {
                var ids = t_learningcentertimeslotsassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_TimeSlots = _T_TimeSlots.Where(p => ids1.Contains(p.T_LearningCenterTimeSlotsAssociationID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_TimeSlots");
           ...
           // return View("Index", _T_TimeSlots.ToPagedList(pageNumber, pageSize));
		    if (IsReports != null)
            {
			  var lstCustReportItem = db.FavoriteItems.Where(p => p.LastUpdatedByUser == User.Name);
                ...
                return PartialView("~/Views/T_TimeSlots/CustomReportResult.cshtml", _T_TimeSlots.ToPagedList(pageNumber, pageSize));
            }
		...
		var entity = "T_TimeSlots";
           ...
           var entitylist = db.T_TimeSlotss;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_TimeSlotss.GroupBy(p => p.t_learningcentertimeslotsassociation.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Learning Center", "Time Slots"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_TimeSlots", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_TimeSlotss.FirstOrDefault(p => p.Id == idvalue);
		 ...
		 public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_TimeSlotss.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_TimeSlotss.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_TimeSlots>(_Obj, "T_TimeSlots", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_TimeSlots> list = db.T_TimeSlotss;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_TimeSlotss;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_TimeSlots), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_TimeSlots, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_TimeSlots>)q);
				   list = ((IQueryable<T_TimeSlots>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_TimeSlotss;
            ...
            list = _fad.FilterDropdown<T_TimeSlots>(User,list, "T_TimeSlots",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_TimeSlots>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
                ...
                ParameterExpression param = Expression.Parameter(typeof(T_TimeSlots), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_TimeSlots).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_TimeSlots), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TimeSlots), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TimeSlots), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TimeSlots), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TimeSlots), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TimeSlots), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_TimeSlots), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_TimeSlots), "p"));
        ...
        {
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_TimeSlots")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_TimeSlots").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_TimeSlots");
                    if (entList != null)
                    {
                        foreach (var prop in entList.Properties.Where(p => p.Name != "DisplayValue" && p.Name != "T_SlotNo"))
                        {
                            long selectedVal = 0;
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_TimeSlots" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Time Slotss";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_LearningCenterTimeSlotsAssociationID":
										 var t_learningcentertimeslotsassociationId = db.T_LearningCenters.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_learningcentertimeslotsassociationId == null)
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_LearningCenterTimeSlotsAssociationID":
										 var strPropertyT_LearningCenterTimeSlotsAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_LearningCenterTimeSlotsAssociationID").Value;
										 ModelReflector.Property propT_LearningCenterTimeSlotsAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_LearningCenter").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_LearningCenterTimeSlotsAssociation);
										 //var elementTypeT_LearningCenterTimeSlotsAssociation = db.T_LearningCenters.ElementType;
										 //var propertyInfoT_LearningCenterTimeSlotsAssociation = elementTypeT_LearningCenterTimeSlotsAssociation.GetProperty(propT_LearningCenterTimeSlotsAssociation.Name);
										 // var t_learningcentertimeslotsassociationId = db.T_LearningCenters.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_LearningCenterTimeSlotsAssociation.GetValue(p, null)) == assovalue);
										 var t_learningcentertimeslotsassociationId = db.T_LearningCenters.Where(propT_LearningCenterTimeSlotsAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_learningcentertimeslotsassociationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_TimeSlots model = new T_TimeSlots();
	...
	break;
					 case "T_LearningCenterTimeSlotsAssociationID":
		 dynamic t_learningcentertimeslotsassociationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_LearningCenterTimeSlotsAssociation = db.T_LearningCenters.ElementType;
			 //var propertyInfoT_LearningCenterTimeSlotsAssociation = elementTypeT_LearningCenterTimeSlotsAssociation.GetProperty(propT_LearningCenterTimeSlotsAssociation.Name);			 
			 //t_learningcentertimeslotsassociationId = db.T_LearningCenters.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_LearningCenterTimeSlotsAssociation.GetValue(p, null)) == columnValue);
			 t_learningcentertimeslotsassociationId = db.T_LearningCenters.Where(propT_LearningCenterTimeSlotsAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_learningcentertimeslotsassociationId = db.T_LearningCenters.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_learningcentertimeslotsassociationId != null)
			model.T_LearningCenterTimeSlotsAssociationID = t_learningcentertimeslotsassociationId.Id;
		  else
			{
				if ((collection["T_LearningCenterTimeSlotsAssociationID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_LearningCenterTimeSlotsAssociationID = objT_LearningCenter.Id;
					}
					catch
					{
						continue;
         ...
         break;
            default:
			 ...
			 if (model.T_StartTime == DateTime.MinValue)
                            model.T_StartTime = DateTime.Now;
        			 if (model.T_EndTime == DateTime.MinValue)
                            model.T_EndTime = DateTime.Now;
        					    if (ValidateModel(model) && string.IsNullOrEmpty(CheckBeforeSave(model)))
                        {
							var result = CheckMandatoryProperties(model);
                            if (result == null || result.Count == 0)
                            {
								db.T_TimeSlotss.Add(model);
								db.SaveChanges();
							}
							else
                            {
		...
		public bool ValidateModel(T_TimeSlots validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_TimeSlotss.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_TimeSlots OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_TimeSlots").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_TimeSlots");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_TimeSlots OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_TimeSlots");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_TimeSlots OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_TimeSlots");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_TimeSlots OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_TimeSlots");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_TimeSlots OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_TimeSlots").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_TimeSlotss.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_TimeSlotss;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_TimeSlots, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_TimeSlots, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_TimeSlots>)q);
		...
		public string CheckBeforeSave(T_TimeSlots t_timeslots)
        {
			var AlertMsg = "";
            // Write your logic here
		...
		public void OnDeleting(T_TimeSlots t_timeslots)
        {
            // Write your logic here
 
		}
        public void OnSaving(T_TimeSlots t_timeslots, ApplicationContext db)
        {
            // Write your logic here
 
        }
		public void AfterSave(T_TimeSlots t_timeslots)
        {
        ...
        public ActionResult GetCalculationValues(T_TimeSlots t_timeslots)
        {
            t_timeslots.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_timeslots.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_TimeSlots"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_TimeSlots t_timeslots, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_TimeSlots t_timeslots, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
