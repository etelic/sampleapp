    ...
    public partial class T_CityController : BaseController
    {
	 ...
	 if(sortBy == "T_CityStateID")
        ...
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_citycountry,string t_citystate, bool? RenderPartial)
        {
			...
			{
				ViewBag.t_citycountry = new SelectList(T_CityCountrylist, "ID", "DisplayValue");
			}
			...
			{
				ViewBag.t_citystate = new SelectList(T_CityStatelist, "ID", "DisplayValue");
			}
			}
			else
			{
			...
			columns.Add("5", "City State");
				...
				if ((RenderPartial == null ? false : RenderPartial.Value))
					return PartialView("~/Views/T_City/CustomReportSearch.cshtml", new T_City());
		...
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_citycountry,string t_citystate , string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports)
        {
			...
			lstT_City = Sorting.Sort<T_City>(lstT_City, SortOrder);
			 ...
			 if (!string.IsNullOrEmpty(GroupByColumn))
            {
                ...
                foreach (string prop in props)
                {
                    ...
                    var asso = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_City").Associations.FirstOrDefault(p => p.AssociationProperty == prop);
                    if (asso != null)
                    {
                        columns.Add(prop);
                        if (iCnt == 1)
                ...
                var grp_T_City = lstT_City;
                var grp_T_CityList = grp_T_City.GroupByMany(groupcolumn);
                var objT_CityList = new List<T_City>();

                foreach (var grp in grp_T_CityList)
                {
                    ...
                    if (Subgroup != null && Subgroup.Count() > 0)
                    {
                        foreach (var itemsub in Subgroup)
                        {
                            ...
                            {
                                var objT_City = new T_City();
                                objT_City = (T_City)item1;
                                objT_City.m_DisplayValue = SecondKey;
                                objT_CityList.Add(objT_City);
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            var objT_City = new T_City();
                            objT_City = (T_City)itemgrp;
                            objT_City.m_DisplayValue = firstKey.ToString();
                            objT_CityList.Add(objT_City);
                        }
                    }
                }
				if (objT_CityList.Count() > 0)
                lstT_City = objT_CityList.AsQueryable();
           ...
           // return View("Index", _T_City.ToPagedList(pageNumber, pageSize));
		    if (IsReports != null)
            {
			  var lstCustReportItem = db.FavoriteItems.Where(p => p.LastUpdatedByUser == User.Name);
                ...
                return PartialView("~/Views/T_City/CustomReportResult.cshtml", _T_City.ToPagedList(pageNumber, pageSize));
            }
	...
	break;
        ...
        public JsonResult GetValidateBeforeSaveProperties(T_City OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_City");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		public string CheckBeforeSave(T_City t_city)
        {
			var AlertMsg = "";
            // Write your logic here
